...
DATA_PATH = "C:\\SimData\\UIUC_ML\\array_foil_UIUC_cuttail2064_T.npz"
...
t_star = np.arange(initial_time, initial_time+numT*inc_time, inc_time)*dt # 1xT(=1)
###
...
xydata = np.hstack((XC_star[0,0,:][:,None], YC_star[0,0,:][:,None]))
...
XI_star = np.reshape(XC_star, [numd, numT, glayer, zone1_i-2*cuttail])[0:BATCH_SIZE,:,:,:]
XI_field = XI_star[:,:,:,:-1] #,np.newaxis
YI_star = np.reshape(YC_star, [numd, numT, glayer, zone1_i-2*cuttail])[0:BATCH_SIZE,:,:,:]
YI_field = YI_star[:,:,:,:-1] #,np.newaxis
XT_star = np.reshape(XC_star, [numd, numT, glayer, zone1_i-2*cuttail])[BATCH_SIZE:numd,:,:,:]
XT_field = XT_star[:,:,:,:-1] #,np.newaxis
YT_star = np.reshape(YC_star, [numd, numT, glayer, zone1_i-2*cuttail])[BATCH_SIZE:numd,:,:,:] #BATCH_SIZE:numd
YT_field = YT_star[:,:,:,:-1] #,np.newaxis
#LI_star = np.reshape(LC_star, [numd, numT, glayer, zone1_i-2*cuttail])[0:BATCH_SIZE,:,:,:]
#LI_field = LI_star[:,:,:,:-1] #,np.newaxis
#LT_star = np.reshape(LC_star, [numd, numT, glayer, zone1_i-2*cuttail])[0:BATCH_SIZE,:,:,:]
#LT_field = LT_star[:,:,:,:-1] #,np.newaxis
UI_star = np.reshape(UC_star, [numd, numT, glayer, zone1_i-2*cuttail])[0:BATCH_SIZE,:,:,:]
UC_field = UI_star[:,:,:,:-1] #,np.newaxis
VI_star = np.reshape(VC_star, [numd, numT, glayer, zone1_i-2*cuttail])[0:BATCH_SIZE,:,:,:]
VC_field = VI_star[:,:,:,:-1] #,np.newaxis
PI_star = np.reshape(PC_star, [numd, numT, glayer, zone1_i-2*cuttail])[0:BATCH_SIZE,:,:,:]
PC_field = PI_star[:,:,:,:-1] #,np.newaxis
UT_star = np.reshape(UC_star, [numd, numT, glayer, zone1_i-2*cuttail])[BATCH_SIZE:numd,:,:,:]
UT_field = UT_star[:,:,:,:-1] #,np.newaxis
VT_star = np.reshape(VC_star, [numd, numT, glayer, zone1_i-2*cuttail])[BATCH_SIZE:numd,:,:,:]
VT_field = VT_star[:,:,:,:-1] #,np.newaxis
PT_star = np.reshape(PC_star, [numd, numT, glayer, zone1_i-2*cuttail])[BATCH_SIZE:numd,:,:,:]
...
Input_field = np.stack([XI_field, YI_field], axis=4) #XC_field, YC_field,
Test_field = np.stack([XT_field, YT_field], axis=4)
...
Field_star = np.stack([UC_field, VC_field, PC_field], axis=4) #, VC_field, PC_field
...
time_inp = np.arange(0,numT,1) #TC_star[0,:]
time_test = np.arange(1,numT,2)
print(time_inp)
...
input_e = tf.keras.Input(shape=(numT, glayer, zone1_i-2*cuttail-1, 2))
pooling_sizeT = 1
pooling_size = 2 
n_ch = 12   #18     
conv1 = layers.Conv3D(n_ch, (2,3,3), activation='elu', padding = 'same')(input_e)
mp1 = layers.MaxPooling3D((pooling_sizeT,pooling_size,pooling_size))(conv1)

conv2 = layers.Conv3D(n_ch*2, (2,3,3), activation='elu', padding = 'same')(mp1)
mp2 = layers.MaxPooling3D((pooling_sizeT,pooling_size,pooling_size))(conv2)
'''
...
output_d = layers.Conv3DTranspose(3, (2,3,3), activation='elu', padding='same')(conv7)
...
unet.fit([train_inp, time_inp], train_env, validation_split=0.1, epochs=5000, verbose=2)
